...
export default function ExportPhotosButton({ type, search, start, end }: Props) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/export-photos?${q.toString()}`)
      if (!res.ok) {
      ...
      a.href = url
      a.download = `Laporan_${type}_foto${dateTag}.xlsx`
      a.click()
