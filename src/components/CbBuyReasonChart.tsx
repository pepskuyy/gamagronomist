...
type StatsData = { total: number; items: Item[] }

type ChartInnerProps = { items: Item[]; palette: string[]; total: number; label?: string }

const ChartInner = dynamic<ChartInnerProps>(() => import('@/components/CommodityChartInner'), {
  ssr: false,
  loading: () => (
...
export default function CbBuyReasonChart({ filterQuery = '' }: { filterQuery?: string }) {
  const [data, setData] = useState<StatsData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch(`/api/cb-stats/buy-reason${filterQuery}`)
      .then(r => r.json())
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
      <div style={{ width: 32, height: 32, border: '3px solid #e2e8f0', borderTop: '3px solid #f59e0b', borderRadius: '50%', animation: 'spin 0.8s linear infinite', marginRight: '0.75rem' }} />
      Memuat statistik alasan beli...
    </div>
  )
  ...
  if (!data || data.total === 0) return (
    <div style={{ textAlign: 'center', padding: '2.5rem', color: 'var(--text-muted)', background: 'var(--surface-hover)', borderRadius: 'var(--radius-md)' }}>
      ...
      <p style={{ marginTop: '0.5rem', fontWeight: 500 }}>Belum ada data alasan pembelian dari Customer Behavior.</p>
    </div>
  )
  ...
  return (
    <div>
      <div style={{ marginBottom: '1rem' }}>
        <h2 style={{ margin: 0, fontSize: '1.2rem' }}>🛒 Alasan Membeli Produk (Customer Behavior)</h2>
        <p style={{ margin: '0.2rem 0 0', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          Berdasarkan <strong>{data.total}</strong> input alasan pembelian dari laporan CB
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 320px) 1fr', gap: '2rem', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
          <ChartInner items={topItems} palette={PALETTE} total={data.total} label="Alasan Beli" />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          {data.items.length > 12 && (
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>
              +{data.items.length - 12} alasan lainnya
            </p>
          )}
        </div>
      </div>
