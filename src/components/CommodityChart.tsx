...
export default function CommodityChart() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
      <div style={{ width: 32, height: 32, border: '3px solid #e2e8f0', borderTop: '3px solid #6366f1', borderRadius: '50%', animation: 'spin 0.8s linear infinite', marginRight: '0.75rem' }} />
      Memuat statistik...
    </div>
  )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          {data.items.length > 12 && (
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>
              +{data.items.length - 12} komoditas lainnya
            </p>
          )}
        </div>
      </div>
